...
void detKeypointsHarris(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    ...
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    cout << "Harris detection with n=" << keypoints.size() << " keypoints detection in " << 1000 * t / 1.0 << " ms"
         << endl;
    if (bVis)
    {
        // visualize keypoints
...
void detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, std::string detectorType, bool bVis)
{
    ...
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency(); // [ms]
    std::cout << detectorType << " detection with n= " << keypoints.size() << " keypoints detection in " << t << " ms"
              << std::endl;
    if (bVis)
    {
        // visualize keypoints
