...
void magnitudeSobel()
{
    ...
    string windowName1 = "imgGray";
    cv::namedWindow(windowName1, 1); // create window
    cv::imshow(windowName1, imgGray);

    string windowName = "Gaussian Blurring 5x5";
    cv::namedWindow(windowName, 1); // create window
    cv::imshow(windowName, img_blurred);

    string windowName2 = "magnitude";
    cv::namedWindow(windowName2, 1); // create window
    cv::imshow(windowName2, magnitude);
